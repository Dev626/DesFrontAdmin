...
export interface segusuarioObtenerEditar_usuario {usuario_id ?: number, sistema_id ?: number, id ?: string, correo ?: string, nombres ?: string, apellido_paterno ?: string, apellido_materno ?: string, empresa_id ?: number, adjunto_foto_id ?: number, estado ?: number, caducidad_clave ?: number};
...
export class pSegusuarioRegistrar {estado : number; mensaje : string};
...
export class ADMUsuarioServiceJPO {
    ...
    constructor(private ohService : OHService){
        this.jpo = ohService.getOH().getJPO("ovnADM","ADM","module.adm","ADMUsuarioServiceImp");
    }
            ...
            response : (rs) => {
                if(call){
                    var out = new pSegusuarioObtenerEditar();
                        if(rs[0] && rs[0][0]){
                            out.usuario = {usuario_id : rs[0][0][0], sistema_id : rs[0][0][1], id : rs[0][0][2], correo : rs[0][0][3], nombres : rs[0][0][4], apellido_paterno : rs[0][0][5], apellido_materno : rs[0][0][6], empresa_id : rs[0][0][7], adjunto_foto_id : rs[0][0][8], estado : rs[0][0][9], caducidad_clave : rs[0][0][10]};
                        }
                        if(rs[1]){
                            out.empresas = [];
                            for(var i = 0; i < rs[1].length; i++){
